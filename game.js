...
let opencards = [];
const users=[{name:"yosef",winers:0,points:0},{name:"moria",winers:0,points:0}]
let toruser=0
let counttarget=[];
...
board.addEventListener("click", (e) => {
  ...
  if(e.target.className=="card hiden"){
    ...
    if(counttarget.length>2){ return}
  ...
  document.getElementById("0").style.backgroundImage="url:https://sites.google.com/site/all18in/_/rsrc/1401806924591/wallpapers/Island.jpg?height=355&width=400"
    ...
    //  b=document.createElement("submit")
    //  b.id='input'
    ...
    ;
      ...
      if(toruser==1){
        toruser=0,
        document.getElementById("name0").style.borderColor="red",
        document.getElementById("name1").style.borderColor="transparent"
        document.getElementById("name0").style.backgroundColor="rgba(255,0,0,0.3)",
        document.getElementById("name1").style.backgroundColor="transparent"}
      else{toruser=1,
        document.getElementById("name1").style.borderColor="red",
        document.getElementById("name0").style.borderColor="transparent",
        document.getElementById("name1").style.backgroundColor="rgba(255,0,0,0.3)",
        document.getElementById("name0").style.backgroundColor="transparent"}
      if(opencards.length%2 ==0){
      setTimeout(() => {
        // opencards[countopen].style.color = "transparent";
        opencards[countopen].className = "card hiden";
        removephoto(opencards[countopen])
        opencards.pop()
        // opencards[countopen-1].style.color = "transparent";
...
ww.addEventListener('click',(e)=>{
  users[0].name=(document.forms[0][0].value!="")?document.forms[0][0].value:users[0].name
  users[1].name=(document.forms[0][1].value!="")?document.forms[0][1].value:users[1].name
...
document.getElementById("playrs").style.gridTemplateColumns=repeatcard
  ...
  // let setgrid=getgrid()
  // document.getElementById("board").gridTemplateColumns=setgrid
  shuffleCards(cards);

  cards.forEach(addCards);
  document.getElementById("board").gridTemplateColumns=getgrid()

  document.getElementById("ask").remove()},400)
...
if(u0>u1){
  return ` ${users[0].name} is a winner!!`
}
else if(u0==u1){
  return  ` ${users[1].name} and ${users[0].name} you are a winner's!!!`
}
